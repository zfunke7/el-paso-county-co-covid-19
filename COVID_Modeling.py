...
def dataHandler(start_date=datetime(2020,3,1), conf_case_delay=7):
    ...
    for date in Case_Data.COUNTY_DATA.keys():
        if not Case_Data.COUNTY_DATA[date][0] == 'u':
            cases.append(Case_Data.COUNTY_DATA[date][0])
        if not Case_Data.COUNTY_DATA[date][1] == 'u':
            deaths.append(Case_Data.COUNTY_DATA[date][1])
    for date in Case_Data.COUNTY_DATA.keys():
...
def NBD_SEIR(today=datetime(2020,3,6), p=720403, e=0, i=1, r=0, 
    ...
    rho_sched = {**init_rho_sched, **rho_sched}
    rho_dates = []
...
def calibrate_NBD_SEIR(cumu_inf, cumu_deaths, death_rate=0.02, death_period=20, 
    ...
    for date in dates:
        cases.append(Case_Data.COUNTY_DATA[date][0])
        death_data.append(Case_Data.COUNTY_DATA[date][1])
    D = out4[6]
    T = out4[0]
    figure()
...
def compare_data_NBD_SEIR(start_date = datetime(2020,3,1), 
    ...
    for date in Case_Data.COUNTY_DATA.keys():
        ...
        data_times.sort()
    for date in dates:
        if Case_Data.COUNTY_DATA[date][0] == 'u':
            cases.append(0)
        else:
            cases.append(Case_Data.COUNTY_DATA[date][0])
        death_data.append(Case_Data.COUNTY_DATA[date][1])
